    ...
    return (
        ...
        >
                ...
                {items.length > 0 && items.map((day, index) => {
                    return (
                        <div className="row mb-3" key={index}>
                            <fieldset id={`meals-fieldset-${index}`}>
            ...
            <Modal.Footer>
                ...
                <button id='close-meals-modal' type="button" className="btn btn-primary" onClick={() => { props.onHide() }}>{props.messages.meals_modal_submit}</button>
            </Modal.Footer>
      </Modal>
    )
}
